...
Массив slug-ов **из каталога** (см. выше), на которые клиент **может сегодня же пойти на собес и получить оффер без переобучения** — по реальному коммерческому опыту в резюме/анкете. Используется scorer-ом для расчёта близости перехода.

**Порог включения — 100% match опыта.** Не «смежная область», не «часто пересекается», не «лид дизайна работает с продуктом, значит может в PM». Если роль требует хоть **какого-то** дообучения / прокачки навыков — она в `currentSlugs` НЕ идёт (для этого есть scorer + Phase 1 анализ).
...
| Cloud Engineer (off-index) | `cloud_engineer` (offIndex) | `[]` (off-index в массив НЕ идёт) |
...
**Зачем:** клиент-HR (не IT) должен в шортлисте увидеть `recruiter`, клиент-менеджер не-IT — `project_manager` / `product_manager`. Иначе ему рекомендуют только overshoot-роли (Data Engineer, ML), куда он реально не дойдёт без переобучения, и теряется самый очевидный шаг.
