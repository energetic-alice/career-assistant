...
- `whyItWorks` — **2-3 предложения, ОБЕ части обязательны для КАЖДОЙ идеи** (не «опция», не «можно опустить»). Идея, в `whyItWorks` которой нет явно названной ценности и привязки к `keyPoints`, считается невалидной — переформулируй до того, как вернёшь JSON.
  ...
  - **Часть 2 (сигнал ценности идеального кандидата):** какую ценность hiring-менеджеров target-индустрии/типа компаний пост демонстрирует **через конкретные действия из `keyPoints`**, а не через декларацию. Шаблон: *«Содержательно пост показывает {ценность-1} (и опционально {ценность-2}): автор {конкретное действие из keyPoints} → {как это связано с тем, что ищут компании}»*. Без шаблона/ценности — невалидная идея.
- `cta` — мягкий CTA в конце поста (комментарий / DM / save). **Никаких** «Open to work, нанимаешь — пиши» в CTA.

**Сигнал ценности идеального кандидата (обязательно в `whyItWorks`):**
