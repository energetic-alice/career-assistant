...
- **Стек / 2-4 ключевые технологии**: только из пересечения `marketKeywords ∩ опыт клиента`. Через запятую. **В headline идут core competencies роли, не serving/infra-tooling.** Принцип: keyword'ы того уровня абстракции, по которому рекрутер ИЩЕТ candidate'а на эту роль.
  - **ML / Data Science / Data Engineer**: фреймворки + тип ML — `Python, TensorFlow, PyTorch, Deep Learning, MLOps, NLP, Computer Vision`. ❌ НЕ в headline: `FastAPI`, `Flask`, `Docker`, `Kubernetes`, `Airflow` (кроме target = `MLOps Engineer` / `ML Platform Engineer` / `Data Platform Engineer`). Эти инструменты остаются в About → Tech stack и в Skills внутри Experience.
  - **Backend**: язык + основной фреймворк/инфра — `Python, Django, PostgreSQL` / `Go, gRPC, Kafka` / `Java, Spring, Kafka`. ❌ НЕ в headline: `Docker`, `Nginx`, `RabbitMQ`.
  - **Frontend**: язык + фреймворк — `React, TypeScript, Next.js` / `Vue, Nuxt, TypeScript`. ❌ НЕ в headline: `Webpack`, `Vite`, `Storybook`, `Jest`.
  - **DevOps / SRE / Platform / Cloud Engineer**: тут инфра-стэк ЕСТЬ core — `Kubernetes, Docker, Terraform, AWS, GitOps` идут в headline.
  - **Mobile**: язык + платформа — `Kotlin, Android, Jetpack Compose` / `Swift, iOS, SwiftUI`.
  - Простая проверка: рекрутер ML ищет `"Python TensorFlow"`, а не `"FastAPI Docker"` (последнее есть у любого backend-инженера и не отличает ML-кандидата). Если технология есть у 80%+ инженеров любой роли — она не headline-keyword.
...
### `seed.top5` — обязательный якорь для headline (если seed есть)

`seed.top5` (приходит во входе через блок `## Market keywords seed`) — это 5 самых частотных рекрутерских keyword'ов под target-роль из централизованного каталога. Это **основа keyword consistency** профиля: Top Skills (Phase 3 секция 4) = `seed.top5` ровно; headline должен дублировать как можно больше из этих 5.

**Правила использования `seed.top5` в headline:**
- **Варианты 1/3/4 (classic / industry / b2b_remote)** — в стэке используй **минимум 2-3 элемента из `seed.top5`** в первую очередь (если они есть у клиента в опыте). Только потом — дополнения из `seed.extended` или core-keyword'ы выше grade-уровня.
- **Вариант 5 (keyword-heavy)** — самый жёсткий: **все 5 элементов `seed.top5` должны попасть в `text`** (если они есть у клиента в опыте). Добавь сверху 1-3 ещё core-keyword'а — `Next.js` / `SSR` / `Accessibility` для Frontend, `Deep Learning` / `MLOps` / `NLP` для ML, и т.п. — **НЕ дополняй tooling-стеком** (`Webpack`, `Jest`, `Storybook`, `Docker`, `FastAPI`).
- **Вариант 2 (achievement)** — главная роль у метрики, поэтому keyword'ов меньше; но **минимум 2 из `seed.top5`** должны остаться.
- **Если `seed.top5` отсутствует** (slug-а нет в каталоге — будет пометка «СИД НЕДОСТУПЕН») — собирай keyword'ы по правилу core-only из Шага 2, без обязательного якоря.

Пример для `frontend_react` (top5 = `JavaScript, TypeScript, React, HTML, CSS`):
- ✅ keyword-heavy: `Senior Frontend Developer | React | TypeScript | JavaScript | HTML | CSS | Next.js | 6+ years` — все 5 + Next.js
- ❌ keyword-heavy: `Senior Frontend Developer | React | TypeScript | Redux Toolkit | Next.js | Jest` — нет JS/HTML/CSS, есть Jest (tooling)

Пример для `ml_engineer` (top5 = `Python, Machine Learning, PyTorch, Deep Learning, MLOps`):
- ✅ keyword-heavy: `ML Engineer | Python | Machine Learning | PyTorch | Deep Learning | MLOps | Computer Vision | Industrial AI`
- ❌ keyword-heavy: `ML Engineer | Python | PyTorch | TensorFlow | MLOps | Deep Learning | Anomaly Detection | Industrial AI` — нет `Machine Learning` (заменено на конкретные алгоритмы), это размывает SEO-якорь

### Правило core-only применяется ко ВСЕМ 5 вариантам без исключений

(см. Шаг 2 «Стек / 2-4 ключевые технологии»). Даже в `keyword_heavy` варианте — нельзя добавлять serving/infra-tooling (`Docker`, `FastAPI`, `Flask`, `Kubernetes`, `Webpack`, `Vite`, `Jest`, `Storybook`, `Airflow`) для ролей где они НЕ core (ML / Data / Backend / Frontend / Mobile / Product / Marketing). Keyword-heavy = больше **core**-keyword'ов через разделители, а НЕ «втащить весь стек включая deployment и testing». Если хочется добавить ещё keyword'ов — бери next-level core: `Deep Learning`, `MLOps`, `NLP`, `Computer Vision`, `Feature Engineering`, `Model Deployment`, `Predictive Maintenance` (для ML); `Microservices`, `Distributed Systems`, `API Design` (для Backend); `Performance Optimization`, `Accessibility`, `SSR`, `Server Components` (для Frontend). Это всё ещё core-уровень абстракции, не tooling.

### Финальная самопроверка перед возвратом ответа

Пройдись по каждому варианту:

1. **Top-5 покрытие.** Если `seed.top5` есть в input'е — посчитай, сколько его элементов попало в `text` варианта 5 (keyword-heavy). Должно быть **5 из 5** (или 4 из 5, если один реально не у клиента в опыте — но запиши это в `whyThis`). Для вариантов 1/3/4 — минимум 2-3. Для варианта 2 — минимум 2.
2. **Запрет tooling.** Если в `keywords` или `text` есть `Docker` / `FastAPI` / `Flask` / `Kubernetes` / `Nginx` / `Webpack` / `Vite` / `Storybook` / `Jest` / `Cypress` / `Airflow` — и при этом target-роль НЕ DevOps / SRE / Platform / Cloud / MLOps Engineer — **удали этот keyword** и замени на seed.top5-элемент (если ещё не использован) или на next-level core-keyword.
3. **Длина ≤ 120 символов** для каждого варианта.
